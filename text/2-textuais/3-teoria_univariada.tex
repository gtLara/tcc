...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.75]{figures/arma_21_pzp.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.75]{figures/arma_21_integrated_pzp.png}
    \caption{Visualização de posicionamento de polos e zeros de processo
...
A presença de raízes unitárias no processamento de sinais determinísticos leva
ao caso de estabilidade marginal. As respostas a sinais determinísticos de
energia são sinais de potência periódicos, estado altamente indesejado no
contexto típico de sistemas de controle mas útil em outros como osciladores
eletrônicos. TODO: ver se resposta ao impulso de sistema simulado acima é
oscilatória mesmo... se sim, fazer o paralelo entre sistema marginal respondendo
a um sinal determinístico e sistema marginal respondendo a ruído branco.


\subsection{Raízes Unitárias no Polinômio Média Móvel}

A presença de uma raiz unitária no polinômio de média móvel de um filtro ARMA
gera efeitos menos dramáticos em sua dinâmica. As
figuras~\ref{fig:ARMA2-1-diff} e~\ref{fig:ARMA2-1-diff} ilustram uma realização
do processo $y_t$ com adição de raiz unitária de média móvel e o diagrama de
polos e zeros resultante, respectivamente.
...
recíproca ao caso normal de raízes unitárias, em que há introdução de
componentes de baixa frequência. Isso será discutido na seção de análise
espectral.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{figures/arma_21_diff.png}
    \caption{Visualização de posicionamento de polos e zeros de processo
...
$$ w_t = (1 - L)y_t = \frac{1 + 0.8L}{1 + 0.5L - 0.25L^2}\varepsilon_t$$

Temos $w_t = y_t - y_{t-1} = \nabla y_t$ tal que $w_t$ seja a derivada de
$y_t$. O ato de diferenciar uma série, em uma tentativa de introduzir
estacionariedade por exemplo, corresponde à introdução de uma raiz unitária no
polinômio de média móvel. Observamos aqui como uma série se
