...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.65]{figures/arma_21_pzp.png}
...
que o diferenciando uma vez temos em um processo estacionário.
...
corretamente que um processo $I(d)$, $d \neq 0$, não é estacionário, já que a
condição de estacionariadade para processos ARMA é pontualmente violada.
TODO: reescrever isso aqui kkk
Interessantemente o sistema, caso inicialmente inversível, não deixa de ser.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{figures/arma_21_integrated_pzp.png}
    \caption{Visualização de posicionamento de polos e zeros de processo
    ARMA(2, 1) em relação ao
    círculo unitário no plano L}
    \label{fig:ARMA2-1}
...
Por fim é interessante observar os paralelos entre a presença de uma raiz
unitária e estabilidade marginal, já que ambos são causados por um polo
presente no círculo unitário. Um sistema no regime de estabilidade marginal
...

TODO: fechar essa ideia
